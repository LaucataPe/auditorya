...
type Informe = {
  id: string
  tipo: 'memo_planeacion'
  contenido: Record<string, string>
  estado: EstadoInforme
  aprobadoAt: string | null
}

const FILAS_LARGAS = new Set(['entendimiento', 'riesgos_significativos', 'control_interno'])

export function MemoPlaneacionTab({
  auditoriaId,
  empresaNombre = '',
  periodo = '',
}: {
  ...
  const { data: informes = [], isLoading } = useQuery<Informe[]>({
    ...
    queryFn: () => api.get<{ tipo: string }[]>(`/auditorias/${auditoriaId}/informes`).then(
      (r) => r.filter((i) => i.tipo === 'memo_planeacion') as Informe[]
    ),
  })

  const informe = informes[0] ?? null
  const aprobado = informe?.estado === 'aprobado'
  const secciones = SECCIONES_INFORME['memo_planeacion']

  const exportOpts = () => ({
    titulo: TIPO_INFORME_LABEL['memo_planeacion'],
    empresaNombre,
    periodo,
    firma,
    secciones: secciones.map((s) => ({ label: s.label, contenido: contenidoLocal[s.key] ?? '' })),
  })
  const nombreArchivo = `Memo_planeacion_${empresaNombre.replace(/[^\w]+/g, '_')}`
  ...
  const generarMutation = useMutation({
    mutationFn: () => api.post<Informe>(`/auditorias/${auditoriaId}/informes/memo_planeacion/generar`, {}),
    onSuccess: (data) => {
  ...
  return (
    <div className="space-y-5">
      {!informe ? (
        /* Sin memo generado aún */
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-200 bg-white py-16 text-center">
          <ClipboardList size={36} className="text-gray-300 mb-3" />
          <p className="text-sm font-medium text-gray-500">Memo de planeación no generado</p>
          <p className="text-xs text-gray-400 mt-1 max-w-sm">
            El sistema lo arma con la materialidad, los riesgos, el control interno y el cronograma que
            ya hayas registrado. Puedes generarlo durante la planeación y completarlo luego.
          </p>
          <Button
            size="sm"
            ...
            <div className="flex items-center gap-2">
              ...
              {user?.rol === 'socio' && (
                aprobado ? (
                  ...
                  >
                    <ShieldCheck size={14} /> Aprobar memo
                  </Button>
                )
              )}
            </div>
            ...
            {secciones.map((sec) => (
              <div key={sec.key} className="bg-white rounded-xl border border-gray-200 p-5">
                ...
                <Textarea
                  ...
                  rows={FILAS_LARGAS.has(sec.key) ? 8 : 4}
                  disabled={aprobado}
                  onChange={(e) =>
