/**
 * Catálogo PUC (Decreto 2650 de 1993) completo: clase, grupo, cuenta y subcuenta.
 * Portado del seeder de contabilidadya. Se importa explícitamente (no va en index.ts)
 * para que no entre al bundle del frontend salvo donde se necesite.
 * Formato compacto: [codigo, nombre]. El nivel se deriva de la longitud del código.
 */
export const PUC_CATALOGO: ReadonlyArray<readonly [string, string]> = [
  ['1', 'ACTIVO'],
  ['2', 'PASIVO'],
  ['3', 'PATRIMONIO'],
  ['4', 'INGRESOS'],
  ...
  ['125035', 'TITULOS'],
  ['125040', 'ACEPTACIONES BANCARIAS O FINANCIERAS 125095 OTROS'],
...
/** Nombre oficial del PUC para un código exacto; null si no existe. */
export function nombrePucCatalogo(codigo: string): string | null {
  if (!_mapa) _mapa = new Map(PUC_CATALOGO)
  return _mapa.get(codigo) ?? null
}

/** true si el código de 1, 2, 4 o 6 dígitos existe en el catálogo oficial. */
export function existeEnPuc(codigo: string): boolean {
  return nombrePucCatalogo(codigo) !== null
}
